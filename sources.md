...
8. **Terpedia PubMed/RDF snapshot.** `terport/ttl/pubmed_terpenes.ttl` is generated by `terport/src/update_from_pubmed.py` from PubMed retrievals. The current local snapshot has 6,500 distinct `PMID:` identifiers and 6,500 `fabio:JournalArticle` records. It is an article-coverage snapshot, not a deduplicated compound list.
